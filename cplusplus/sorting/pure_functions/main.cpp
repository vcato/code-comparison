...
namespace pure {
  ...
  static vector<int> concat(const vector<int> &a,const vector<int> &b,const vector<int> &c)
  {
    return concat(concat(a,b),c);
  }


  static vector<int> sorted(const vector<int> &values)
  {
    if (values.size()<2) return values;

    size_t middle_index = values.size()/2;
    int pivot = values[middle_index];
    auto lower = sorted(values_less_than(pivot,values));
    auto middle = values_equal_to(pivot,values);
    auto upper = sorted(values_greater_than(pivot,values));
...
{
  assert(pure::sorted(values)==expected);
}
